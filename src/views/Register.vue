    ...
    <ion-content>
      <div class="info">
        ...
        <ion-item lines="none">
          ...
          <ion-button class="eyebtn" @click.self="ShowHideConfirmPass()">
            <ion-icon slot="icon-only" :icon="EyeIcon" color="primary"></ion-icon>
          </ion-button>
        </ion-item>
      </div>
      ...
      if (this.Password.length >= 8) {
        if (this.Password == this.ConfirmPassword) {
          ...
          this.Accounts.push({
            ...
            mobile: this.Simnumber.replace(/[()]/g, '')
              .replace(/\s/g, '')
              .replace('+', ''),
          ...
          setTimeout(() => {
            localStorage.setItem(
              ...
              JSON.stringify([
                {
                  id: this.Accounts.length + 1,
                  phone_number: this.Simnumber.replace(/[()]/g, '')
                    .replace(/\s/g, '')
                    .replace('+', ''),
                  firstname: this.Firstname,
                  lastname: this.Lastname,
          ...
          this.openToast('Password must be at least 8 characters.');
        }
      } else {
        this.openToast('Password must be at least 8 characters.');
      }
