<template>
    <ion-page>
        ...
        <ion-footer>
            <div class="footer">
                <ion-button mode="ios" @click="SaveAddress()" expand="full" color="primary" :disabled="Status">Save address</ion-button>
            </div>
        </ion-footer>
    </ion-page>
</template>
    ...
    methods: {
        LoadGoogleMap() {
            setTimeout(() => {
                this.Map =  new window.google.maps.Map(document.getElementById("mapDivRef"), {
                        ...
                        scaleControl: false,
                        center: { lat: Number(this.coordlat), lng: Number(this.coordlng) }
                    });
        ...
        SaveAddress() {
            ...
            if (distance <= 50) {
                ...
                axios.post(Global.methods.GetURL() + "/address_create", {
                    userid: this.UserID,
